...
from dl_utils.component_store import ComponentStore
from dl_utils.misc import unsqueeze_right

NoiseSchedules = ComponentStore(
    "NoiseSchedules", "Functions to generate noise schedules"
)


@NoiseSchedules.add_def("linear_beta", "Linear beta schedule")
def _linear_beta(
    num_train_timesteps: int, beta_start: float = 1e-4, beta_end: float = 2e-2
):
    """
...
class Scheduler(nn.Module):
    ...
    def add_noise(
        ...
        noisy_samples = (
            sqrt_alpha_cumprod * original_samples + sqrt_one_minus_alpha_prod * noise
        )
